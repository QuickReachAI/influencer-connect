...
import { lookupYouTubeChannel, apifyLimiter, apifyGlobalLimiter } from '@/lib/services/apify.service';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!handle || typeof handle !== 'string' || handle.trim().length === 0) {
      return NextResponse.json({ error: 'YouTube channel handle or URL is required' }, { status: 400 });
    }
    ...
    const channel = await lookupYouTubeChannel(handle);

    return NextResponse.json({ channel });
  } catch (error: any) {
    console.error('YouTube lookup error:', error);
    const message = error.message || 'Failed to look up YouTube channel';
